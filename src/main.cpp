...
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) + nHeight;

    if (nHeight <= 525600)
        nSubsidy = nCoinAge * (COIN_YEAR_REWARD * 2) * 33 / (365 * 33 + 8) + nHeight;
    else if (nHeight > 525600)
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) + nHeight;;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "UltimateSecureCash", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    };
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
        ...
        || (nNodeMode != NT_FULL && mapBlockThinIndex.empty()))
    {
        ...
        block.nNonce   = !fTestNet ? 816452253 : 816452253;
/*
        ...
        assert(block.hashMerkleRoot == uint256("0x269277d971a47872328ec3c009ea778b72e9b5f00dd44af7ccd35b17d61c8d60"));
